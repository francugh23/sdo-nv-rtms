"use client";

import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ColumnDef } from "@tanstack/react-table";
import { format } from "date-fns";
import { ArrowUpDown } from "lucide-react";

export type Documents = {
  DID: string;
  DocumentTitle: string;
  Status: string;
  Location: string;
  DateTime: Date;
};

export const data: Documents[] = [
  {
    DID: "DA001-02-21-25",
    DocumentTitle: "To attend MANCOM",
    Status: "Pending",
    Location: "Records Unit Office",
    DateTime: new Date(),
  },
  {
    DID: "DA002-02-21-25",
    DocumentTitle: "To attend MANCOM",
    Status: "Pending",
    Location: "Records Unit Office",
    DateTime: new Date(),
  },
  {
    DID: "DA003-02-21-25",
    DocumentTitle: "To attend MANCOM",
    Status: "Pending",
    Location: "Records Unit Office",
    DateTime: new Date(),
  },
];

export const columns: ColumnDef<Documents>[] = [
  {
    accessorKey: "id",
    header: () => <div className="text-center">#</div>,
    cell: ({ row }) => <div className="text-center">{row.index + 1}</div>,
  },
  {
    accessorKey: "DID",
    header: () => <div className="text-center">Document ID</div>,
    cell: ({ row }) => (
      <div className="text-center">{row.getValue("DID")}</div>
    ),
  },
  {
    accessorKey: "DocumentTitle",
    header: () => <div className="text-center">DocumentTitle</div>,
    cell: ({ row }) => <div className="text-center">{row.getValue("DocumentTitle")}</div>,
  },
  {
    accessorKey: "Status",
    header: "Status",
    cell: (row) => {
      return (
        <Badge
          className={
            (row.getValue() as string) === "Pending"
              ? "font-bold bg-yellow-50 border border-yellow-200 text-yellow-800 hover:bg-yellow-100"
              : (row.getValue() as string) === "Received"
              ? "font-bold bg-green-50 border border-green-200 text-green-800 hover:bg-green-100"
              : "font-bold bg-red-50 border border-red-200 text-red-800 hover:bg-red-100"
          }
        >
          {row.getValue() as string}
        </Badge>
      );
    },
  },
  {
    accessorKey: "Location",
    header: "Location",
  },
  {
    accessorKey: "DateTime",
    header: ({ column }) => {
      return (
        <div className="text-right">
          <Button
            variant="ghost"
            onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
          >
            Date & Time
            <ArrowUpDown className="ml-1 h-3 w-3" />
          </Button>
        </div>
      );
    },
    cell: ({ row }) => (
      <div className="text-right">
        {format(new Date(row.getValue("DateTime")), "MMM dd, yyyy hh:mm a")}
      </div>
    ),
  },
];
